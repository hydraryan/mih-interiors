...
const navItems = [
  ...
  { href: urls.admin('/security'), label: 'Security', description: 'Change admin password', icon: KeyRound },
]
...
function getSectionSubtitle(pathname: string) {
  ...
  if (pathname.startsWith('/admin/security')) return 'Change dashboard password and keep admin access secure.'
    ...
    <div className="min-h-screen bg-[#fcf8f5] text-charcoal-900">
          ...
          <nav className="space-y-2">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  {isActive && <div className="absolute inset-0 bg-linear-to-r from-brown-50/50 to-transparent" />}
                  <div className={`relative mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl transition-all duration-300 ${isActive ? 'bg-charcoal-900 text-white shadow-md' : 'bg-white text-charcoal-400 shadow-sm group-hover:bg-brown-50 group-hover:text-brown-700'}`}>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          {/* Menu Drawer */}
          <div className="absolute inset-y-0 left-0 z-10 w-[88%] max-w-sm overflow-y-auto border-r border-white/10 bg-[#f6efe6] p-5 shadow-2xl shadow-charcoal-900/20">
