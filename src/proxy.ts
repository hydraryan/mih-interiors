...
const PUBLIC_ADMIN_PATHS = new Set(['/admin/login', '/admin/login/forgot-password', '/login', '/login/forgot-password'])
...
export async function proxy(request: NextRequest) {
  ...
  const isRequestingAdminUrl = pathname.startsWith('/admin') || pathname.startsWith('/login')
  ...
  if (requiresAuth) {
    ...
    try {
      ...
      console.error('Middleware getToken error:', err instanceof Error ? err.message : err)
      token = null
    }
