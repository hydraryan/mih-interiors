...
export default function ThreeDLeadsPage() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        setLeads(data.filter((l: Lead) => l.source === '3d-rendering'))
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Total 3D Leads', value: leads.length, icon: Box, color: 'bg-charcoal-900' },
      ...
      <div className="flex flex-col gap-8 lg:flex-row">
        ...
        <div className="flex-1 space-y-4">
          <div className="relative flex-1 rounded-3xl border border-white/60 bg-white/80 p-4 backdrop-blur-md shadow-sm">
            <Search className="absolute left-8 top-1/2 h-4 w-4 -translate-y-1/2 text-charcoal-400" />
            <input 
              type="text"
              placeholder="Search 3D leads..."
          ...
          <div className="space-y-3">
            {loading ? (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-charcoal-500">
                        ...
                        <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> {format(new Date(lead.createdAt), 'MMM d')}</span>
                      </div>
                    </div>

                    <div className="flex flex-col items-end gap-2 text-right">
                      <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-brown-700 bg-brown-50 px-2 py-1 rounded-md">
                        {lead.projectType || '3D Design'}
                      </span>
              ...
              <div className="flex flex-col items-center justify-center rounded-[2rem] border border-dashed border-charcoal-900/10 py-20 text-charcoal-500">
                ...
                <p className="font-display text-xl">No 3D leads found</p>
              </div>
            )}
          </div>
        </div>
            ...
            {selectedLead ? (
              <div className="space-y-8 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
                <div>
                  ...
                  <p className="mt-2 text-charcoal-500 uppercase text-[10px] font-bold tracking-widest">3D Visualization Request</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-6">
                  ...
                  {selectedLead.projectType && <DetailItem icon={Briefcase} label="Project Type" value={selectedLead.projectType} />}
                  {selectedLead.areaSqft && <DetailItem icon={Layout} label="Area" value={`${selectedLead.areaSqft} sq.ft.`} />}
                </div>
              ...
              <div className="flex h-96 flex-col items-center justify-center text-center text-charcoal-400">
                ...
                <p className="mt-2 text-sm max-w-[200px]">Select a lead to view their 3D request details.</p>
              </div>
            )}
          </div>
        </div>
