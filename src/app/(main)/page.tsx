...
export default async function Home() {
  let media: any
  let homepageServices: any[]

  try {
    const conn = await dbConnect()
    if (conn) {
      ;[media, homepageServices] = await Promise.all([
        getActiveMediaMap(),
        Service.find({ showOnHomepage: true }).sort({ order: 1 }).lean(),
      ])
    } else {
      throw new Error('DB connection skipped')
    }
  } catch (err) {
    console.warn('DB unavailable — using fallback data for homepage.', err?.message || err)
    media = { resolve: (p: string) => p }
    homepageServices = []
  }
